...
export const RULES = [
  {
    ...
    dm: (c) => `Bonjour ${c.name}, on regarde ça tout de suite 👀\n\n` +
      (c.ref
        ...
        : '') +
      `Le suivi se fait sur WhatsApp, où votre commande a été confirmée — on vous donne l'étape exacte (atelier, expédition) en direct : ${wa(`Bonjour Djimmy Prints, où en est ma commande ${c.ref || ''} ?`.trim())}`,
    comment: (c) => `Bonjour ${c.name} 👋 Écrivez-nous sur WhatsApp au ${PHONE_DISPLAY}${c.ref ? ` avec votre référence ${c.ref}` : ''}, on vous donne l'état exact de votre commande tout de suite.`,
  },
  {
    key: 'prix',
    label: 'Prix / tarif',
    ...
    dm: (c) => `Bonjour ${c.name} 👋 Merci pour votre message !\n\n` +
      ...
      `• Commander en ligne : ${SITE}/commande\n\n` +
  ...
  },
  {
    ...
    dm: (c) => `Bonjour ${c.name}, avec plaisir 📄\n\n` +
      `Le devis est gratuit et sans engagement. Il nous faut : le vêtement, la quantité, la technique de marquage et votre wilaya.\n\n` +
      `Envoyez aussi votre logo si vous l'avez (SVG ou PNG 300 dpi de préférence), on vous répond avec le prix ferme : ${c.links.devisWa}\n\n` +
      `Le catalogue avec les prix : ${SITE}/catalogue`,
    comment: (c) => `Bonjour ${c.name} 👋 Devis gratuit : envoyez-nous quantité + vêtement sur WhatsApp ${PHONE_DISPLAY} et on vous chiffre ça 📄 Prix indicatifs : ${SITE}/catalogue`,
  },
  {
    key: 'minimum',
  ...
  },
  {
    ...
    dm: (c) => `Bonjour ${c.name} ⏱\n\n` +
      `Comptez en général 3 à 7 jours ouvrables entre la validation du bon à tirer et la livraison, selon votre wilaya et la complexité du marquage. Les urgences se traitent au cas par cas — dites-nous votre date butoir.\n\n` +
      `Écrivez-nous la quantité et la date souhaitée, on vous confirme le planning : ${c.links.wa}`,
    comment: (c) => `Bonjour ${c.name} 👋 En général 3 à 7 jours ouvrables selon la wilaya et le marquage. Urgence possible — écrivez-nous sur WhatsApp ${PHONE_DISPLAY} ⏱`,
  },
  {
    key: 'livraison',
    label: 'Livraison',
    ...
    dm: (c) => `Bonjour ${c.name} 🚚\n\n` +
      `Nous livrons dans les 58 wilayas, à domicile ou en stop desk. Les frais et le délai dépendent de votre wilaya et du volume ; à Alger, remise en main propre possible à l'atelier (${ADDRESS}).\n\n` +
  ...
  },
  {
    ...
    dm: (c) => `Bonjour ${c.name} 🧵\n\n` +
      ...
      `Envoyez-nous votre logo en haute résolution (SVG, ou PNG 300 dpi minimum) et on vous dit la meilleure technique. Si votre logo n'est pas vectorisé, on peut s'en charger : ${c.links.wa}`,
    comment: (c) => `Bonjour ${c.name} 👋 Broderie, sérigraphie, transfert, sublimation et flocage 🧵 Envoyez-nous votre logo sur WhatsApp ${PHONE_DISPLAY}, on vous conseille la bonne technique.`,
  },
  {
    key: 'echantillon',
    label: 'Échantillon / réalisations',
    hint: '« Vous avez des photos de vos travaux ? »',
    keywords: [
      'echantillon', 'exemple', 'realisation', 'realisations', 'reference client', 'portfolio',
      'photo de vos', 'vos travaux', 'deja fait', 'sample', 'نموذج', 'عينة', 'صور',
    ],
    strong: ['echantillon', 'realisation', 'portfolio', 'عينة'],
    dm: (c) => `Bonjour ${c.name} 📸\n\n` +
      `Avec plaisir : dites-nous le vêtement et le secteur (restaurant, hôtel, chantier, clinique…) et on vous envoie des photos de réalisations similaires, avant tout engagement.\n\n` +
      `Sur demande, un échantillon peut aussi être produit avant le lancement d'une grosse série : ${c.links.wa}`,
    comment: (c) => `Bonjour ${c.name} 👋 Écrivez-nous sur WhatsApp ${PHONE_DISPLAY} : on vous envoie des photos de réalisations dans votre secteur 📸`,
  },
  {
    key: 'catalogue',
  ...
  },
  {
    ...
    dm: (c) => `Bonjour ${c.name}, super 🙌\n\n` +
      ...
      `• L'assistant de commande en 3 étapes (produits → technique & logo → livraison) : ${SITE}/commande — il calcule la remise et prépare votre message.\n` +
