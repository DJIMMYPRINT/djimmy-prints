...
const GALLERY = [
  { src: `${SB}/photo1.jpeg`, label: 'Polo personnalisé · Logo brodé professionnel', tag: 'Broderie' },
  { src: `${SB}/photo2.jpeg`, label: 'Uniforme complet · Impression haute qualité', tag: 'Transfert numérique' },
  { src: `${SB}/photo3.jpeg`, label: 'Tenue de travail · Logo sérigraphié', tag: 'Sérigraphie' },
  { src: `${SB}/photo4.jpeg`, label: 'Polo corporate · Personnalisation complète', tag: 'Broderie' },
  { src: `${SB}/photo5.jpeg`, label: 'Uniforme professionnel · Livraison rapide', tag: 'Transfert numérique' },
  { src: `${SB}/photo6.jpeg`, label: 'Tenue industrielle · Multi-poches', tag: 'Sérigraphie' },
]
...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* HERO */}
      <section style={{minHeight:'100vh',display:'flex',alignItems:'center',padding:'9rem 4vw 5rem',position:'relative',overflow:'hidden'}}>
        <div style={{maxWidth:860,position:'relative',zIndex:1}}>
          <div style={{display:'inline-flex',alignItems:'center',gap:'.6rem',background:'var(--green-pale)',border:'1px solid rgba(45,90,39,.2)',padding:'.4rem 1rem',borderRadius:'100px',fontSize:'.72rem',fontWeight:600,letterSpacing:'.12em',textTransform:'uppercase',color:'var(--green)',marginBottom:'2rem'}}>
          ...
          <h1 style={{fontFamily:'Anton',fontWeight:400,fontSize:'clamp(2.8rem,8vw,6.2rem)',lineHeight:1.03,textTransform:'uppercase',letterSpacing:'.005em',marginBottom:'1.8rem',color:'var(--black)'}}>
            Fait pour ceux<br/>
            <span style={{color:'var(--green)'}}>qui rêvent grand.</span>
          </h1>
          <p style={{fontSize:'1.05rem',color:'var(--muted)',lineHeight:1.85,maxWidth:500,marginBottom:'2.8rem'}}>
          ...
          <div style={{display:'flex',gap:'1rem',flexWrap:'wrap',marginBottom:'4rem'}}>
            <Link href="/commande" className="btn-g">Commander maintenant</Link>
            <Link href="/catalogue" className="btn-outline">Voir le catalogue</Link>
          </div>
          <div style={{display:'flex',gap:'3rem',paddingTop:'2.2rem',borderTop:'1.5px solid var(--cream-border)',flexWrap:'wrap'}}>
        ...
        <div className="mqt">
          {[...Array(2)].map((_,i) => (
            ['Broderie','Sérigraphie','Transfert Numérique','Sublimation','Flocage','Livraison Nationale','Devis 24H'].map(item => (
              <span key={`${i}-${item}`} className="mqi">{item}<span className="mqd"/></span>
            ))
          ))}
        </div>
      </div>
      ...
      <section ref={rv(0)} className="rv" style={{padding:'5.5rem 4vw',background:'rgba(255,255,255,.5)',backdropFilter:'blur(6px)',position:'relative',zIndex:1}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:'1.5rem',marginTop:'3rem'}}>
          {SERVICES.map((s,i) => (
            <div key={s.name} ref={rv(10+i)} className="rv" style={{background:'var(--cream)',border:'1.5px solid var(--cream-border)',padding:'2.2rem 1.8rem',borderRadius:'6px',transition:'all .3s',transitionDelay:`${i*0.08}s`}}
      ...
      <section ref={rv(1)} className="rv" style={{padding:'5.5rem 4vw',position:'relative',zIndex:1}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:'1.5rem',marginTop:'3rem'}}>
          {WHY.map((w,i) => (
            <div key={w.n} ref={rv(20+i)} className="rv" style={{padding:'1.8rem 1.5rem',border:'1.5px solid var(--cream-border)',borderRadius:'6px',background:'var(--white)',transition:'all .3s',transitionDelay:`${i*0.08}s`}}
      ...
      {/* CTA BAND */}
      <section ref={rv(2)} className="rv" style={{background:'var(--green)',padding:'4rem 4vw',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'2rem',flexWrap:'wrap',position:'relative',overflow:'hidden',zIndex:1}}>
        <div style={{position:'absolute',right:'-1rem',top:'50%',transform:'translateY(-50%)',fontFamily:'Anton',fontSize:'7rem',fontWeight:400,color:'rgba(255,255,255,.07)',whiteSpace:'nowrap',pointerEvents:'none'}}>DJIMMY PRINTS</div>
        <div>
          <div style={{fontFamily:'Anton',fontWeight:400,fontSize:'clamp(1.6rem,3.5vw,2.5rem)',textTransform:'uppercase',color:'var(--cream)',lineHeight:1}}>Prêt à habiller votre équipe ?</div>
          <p style={{fontSize:'.88rem',color:'rgba(245,240,232,.75)',marginTop:'.4rem'}}>Devis gratuit · Réponse sous 24h · Livraison dans 58 wilayas</p>
        </div>
        <div style={{display:'flex',gap:'1rem',flexWrap:'wrap',position:'relative',zIndex:1}}>
          <Link href="/commande" style={{background:'var(--cream)',color:'var(--green)',padding:'.9rem 2.2rem',fontWeight:700,fontSize:'.85rem',letterSpacing:'.07em',textTransform:'uppercase',borderRadius:'3px',textDecoration:'none',display:'inline-block'}}>Commander</Link>
          <a href={`https://wa.me/${WA}?text=${WA_MSG}`} target="_blank" rel="noopener noreferrer" style={{background:'transparent',color:'var(--cream)',padding:'.9rem 2.2rem',fontWeight:600,fontSize:'.85rem',letterSpacing:'.07em',textTransform:'uppercase',border:'1.5px solid rgba(245,240,232,.4)',borderRadius:'3px',textDecoration:'none',display:'inline-block'}}>💬 WhatsApp</a>
        </div>
      </section>
      ...
      <section ref={rv(5)} className="rv" style={{padding:'5.5rem 4vw',background:'rgba(255,255,255,.5)',backdropFilter:'blur(6px)',position:'relative',zIndex:1}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:'1rem',marginTop:'3rem'}}>
          {GALLERY.map((p,i) => (
            <div key={i} ref={rv(40+i)} className="rv" style={{borderRadius:'8px',overflow:'hidden',border:'1.5px solid var(--cream-border)',background:'var(--white)',transition:'all .3s',transitionDelay:`${i*0.07}s`}}
              ...
              <div style={{aspectRatio:'4/3',overflow:'hidden',background:'var(--cream)'}}>
                <img src={p.src} alt={p.label} style={{width:'100%',height:'100%',objectFit:'cover',transition:'transform .4s'}}
      ...
      <section ref={rv(3)} className="rv" style={{padding:'5.5rem 4vw',position:'relative',zIndex:1}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:'1.5rem',marginTop:'3rem'}}>
          {TESTIMONIALS.map((t,i) => (
            <div key={i} ref={rv(30+i)} className="rv" style={{background:'var(--white)',border:'1.5px solid var(--cream-border)',borderRadius:'6px',padding:'2rem',transition:'all .3s',transitionDelay:`${i*0.1}s`}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:'.8rem'}}>
                <div style={{width:38,height:38,background:'var(--green)',borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',color:'var(--white)',fontWeight:700,fontSize:'.9rem',flexShrink:0}}>{t.author[0]}</div>
                <div>
